...
interface Props {
  title?: string
  allowUpload?: boolean // 印影など画像アップロードを許可
  onCancel: () => void
  onConfirm: (dataUrl: string) => void
}

/** 手書きサイン/印影をキャンバス描画または画像アップロードで取得するモーダル */
export default function SignaturePadModal({ title = '署名を描いてください', allowUpload = false, onCancel, onConfirm }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-5 space-y-3">
        ...
        <p className="text-xs text-gray-400 text-center">枠内に指またはマウスで描いてください</p>
        {allowUpload && (
